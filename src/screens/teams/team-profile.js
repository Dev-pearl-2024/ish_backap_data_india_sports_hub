...
export default function TeamProfile({route, params}) {
  ...
  const getTeamProfile = async () => {
    try {
      ...
      setLoading(false);
      console.log('team profile data', res?.data?.existingTeam);
  ...
  const renderComponent = (
    <ScrollView alwaysBounceVertical showsVerticalScrollIndicator={false}>
      ...
      {activeTab === 5 && <LatestNews showTitle={false} sportData={athProfileData?.sports}/>}
      <ScrollView
        horizontal
        showsHorizontalScrollIndicator={false}
        contentContainerStyle={{padding: 16, gap: 6}}>
      ...
      {activeTab === 3 && activeArchiveTab === 0 && (
        <RecentArchives teamId={athProfileData?._id} />
      )}
