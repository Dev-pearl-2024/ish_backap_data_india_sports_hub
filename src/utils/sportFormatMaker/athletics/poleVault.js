export const ConvertPoleVault = (rawData) => {
    const heights = rawData?.[1]?.slice(4, 14); // extract jump heights
    ...
    const output = rawData.slice(2).map((row, index) => {
        ...
        return {
            ...
            heights,
            bestJump,
            note: row[row.length - 1], // customize if needed
            attempts
        };
    });
    return output
