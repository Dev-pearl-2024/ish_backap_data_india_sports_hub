...
import RoundRobinDraws from './RoundRobin';


const PyramidAndRoundRobinDiff = ({ sportData }) => {
    ...
    return (
        <View style={styles.container}>
            {console.log("PYRAMID&RR", data?.existing?.data[0].competitionType)}
            {data?.existing ? (
                data?.existing?.data?.map((it) => {
                    it.drawsId = data?.existing?._id
                    if (it.competitionType === 'round-robin') {
                        return <RoundRobinDraws sportData={it} />
                    } else {
                        return <InvertedPyramidDraws sportData={it} />
                    }
                })
            ) : (
...
export default PyramidAndRoundRobinDiff;
